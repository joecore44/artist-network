...
class Show(db.Model):
  ...
  db.relationship('Artist', backref='Artist', lazy=True)
...
def venues():
  ...
  '''
  data = []
  citie_state = set() #set the city and state
  for venue in live_venues:
      citie_state.add((venue.city, venue.state))  # Add the tuple

  citie_state = list(citie_state)
  # Sorts on second column first (state), then by city.
  citie_state.sort(key=itemgetter(1, 0))

  today = datetime.now()

  for loc in citie_state:
...
def show_venue(venue_id):
  ...
  live_venue = Venue.query.get(venue_id)
  data1={
    "id": 1,
    "name": "The Musical Hop",
  ...
  if venue_id == 1:
    data = list(filter(lambda d: d['id'] == venue_id, [live_venue, data2, data3]))[0]
  else:
...
@app.route('/artists')
def artists():
  live_data = Artist.query.all()
  # TODO: replace with real data returned from querying the database
  return render_template('pages/artists.html', artists=live_data)
...
def show_artist(artist_id):
  ...
  live_artist = Artist.query.get(artist_id)
  ...
  #data = list(filter(lambda d: d['id'] == artist_id, [live_artist, data2, data3]))[0]
  return render_template('pages/show_artist.html', artist=live_artist)
